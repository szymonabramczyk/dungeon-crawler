...
void Assets::loadAssets(const std::string& path) {
    ...
    // a loop to automatically load sprites with given names into the sprite map
    for (const auto& sprite_name : sprites_names) {
        std::filesystem::path file = cwd / sprite_name;
        std::string::size_type pos = sprite_name.find('.');
        if (pos != std::string::npos) {
            std::string tmp = sprite_name.substr(0, pos);
            loadSprite(tmp, file); 
        }
    }
    // a loop to automatically load fonts with given names into the sprite map
    ...
    // a loop to automatically load sounds with given names into the sprite map
...
void Assets::loadSprite(const std::string& name, const std::string& filePath) {
    std::unique_ptr<SpriteInfo> sprite(new SpriteInfo(filePath));
    sprites.insert(std::make_pair(name, std::move(sprite)));
}
